...
#ifndef WIN32
 int GetSockLastError( ) { return errno; }
#else
 int GetSockLastError( ) { return GetLastError(); }
#endif
...
void CSocket :: Allocate( int type )
{
	...
	{
		m_HasError = true;
		m_Error = GetSockLastError( );
		CONSOLE_Print( "[SOCKET] error (socket) - " + GetErrorString( ) , true);
		return;
	}
}
...
void CTCPSocket :: DoRecv( fd_set *fd )
{
	...
	if( FD_ISSET( m_Socket, fd ) )
	{
		...
		if( c == SOCKET_ERROR && GetSockLastError( ) != EWOULDBLOCK )
		{
			// receive error

			m_HasError = true;
			m_Error = GetSockLastError( );
			CONSOLE_Print( "[TCPSOCKET] error (recv) - " + GetErrorString( ) , true);
			return;
		}
		else if( c == 0 )
		{
			...
			CONSOLE_Print( "[TCPSOCKET] closed by remote host", true );
			m_Connected = false;
		}
		else if( c > 0 )
		{
...
void CTCPSocket :: DoSend( fd_set *send_fd )
{
	...
	if( FD_ISSET( m_Socket, send_fd ) )
	{
		...
		if( s == SOCKET_ERROR && GetSockLastError( ) != EWOULDBLOCK )
		{
			// send error

			m_HasError = true;
			m_Error = GetSockLastError( );
			CONSOLE_Print( "[TCPSOCKET] error (send) - " + GetErrorString( ) , true);
			return;
		}
		else if( s > 0 )
		{
...
void CTCPClient :: Connect( string localaddress, string address, uint16_t port )
{
	...
	if( !localaddress.empty( ) )
	{
		...
		if( ::bind( m_Socket, (struct sockaddr *)&LocalSIN, sizeof( LocalSIN ) ) == SOCKET_ERROR )
		{
			m_HasError = true;
			m_Error = GetSockLastError( );
			CONSOLE_Print( "[TCPCLIENT] error (bind) - " + GetErrorString( ) , true);
			return;
		}
	}
	...
	if( !HostInfo )
	{
		m_HasError = true;
		// m_Error = h_error;
		CONSOLE_Print( "[TCPCLIENT] error (gethostbyname)" , true);
		return;
	}
	...
	{
		if( GetSockLastError( ) != EINPROGRESS && GetSockLastError( ) != EWOULDBLOCK )
		{
			// connect error

			m_HasError = true;
			m_Error = GetSockLastError( );
			CONSOLE_Print( "[TCPCLIENT] error (connect) - " + GetErrorString( ) , true);
			return;
		}
	}
...
bool CTCPServer :: Listen( string address, uint16_t port )
{
	...
	if( ::bind( m_Socket, (struct sockaddr *)&m_SIN, sizeof( m_SIN ) ) == SOCKET_ERROR )
	{
		m_HasError = true;
		m_Error = GetSockLastError( );
		CONSOLE_Print( "[TCPSERVER] error (bind) - " + GetErrorString( ) , true);
		return false;
	}
	...
	if( listen( m_Socket, 8 ) == SOCKET_ERROR )
	{
		m_HasError = true;
		m_Error = GetSockLastError( );
		CONSOLE_Print( "[TCPSERVER] error (listen) - " + GetErrorString( ) , true);
		return false;
	}
...
bool CUDPSocket :: SendTo( string address, uint16_t port, BYTEARRAY message )
{
	...
	if( !HostInfo )
	{
		m_HasError = true;
		// m_Error = h_error;
		CONSOLE_Print( "[UDPSOCKET] error (gethostbyname)" , true);
		return false;
	}
...
bool CUDPSocket :: Broadcast( uint16_t port, BYTEARRAY message )
{
	...
	if( sendto( m_Socket, MessageString.c_str( ), MessageString.size( ), 0, (struct sockaddr *)&sin, sizeof( sin ) ) == -1 )
	{
		CONSOLE_Print( "[UDPSOCKET] failed to broadcast packet (port " + UTIL_ToString( port ) + ", size " + UTIL_ToString( MessageString.size( ) ) + " bytes)", true );
		return false;
	}
...
void CUDPSocket :: SetBroadcastTarget( string subnet )
{
	if( subnet.empty( ) )
	{
		CONSOLE_Print( "[UDPSOCKET] using default broadcast target", true );
		m_BroadcastTarget.s_addr = INADDR_BROADCAST;
	}
	else
	{
		...
		CONSOLE_Print( "[UDPSOCKET] using broadcast target [" + subnet + "]" , true);
		...
		{
			CONSOLE_Print( "[UDPSOCKET] invalid broadcast target, using default broadcast target" , true);
			m_BroadcastTarget.s_addr = INADDR_BROADCAST;
		}
	}
}
...
bool CUDPServer :: Bind( struct sockaddr_in sin )
{
	...
	if( ::bind( m_Socket, (struct sockaddr *)&m_SIN, sizeof( m_SIN ) ) == SOCKET_ERROR )
	{
		m_HasError = true;
		m_Error = GetSockLastError( );
		CONSOLE_Print( "[UDPSERVER] error (bind) - " + GetErrorString( ) , true);
		return false;
	}
...
void CUDPServer :: RecvFrom( fd_set *fd, struct sockaddr_in *sin, string *message )
{
	...
	if( FD_ISSET( m_Socket, fd ) )
	{
		...
		if( c == SOCKET_ERROR && GetSockLastError( ) != EWOULDBLOCK )
		{
			...
			m_HasError = true;
			m_Error = GetSockLastError( );
			CONSOLE_Print( "[UDPSERVER] error (recvfrom) - " + GetErrorString( ), true );
		}
		else if( c > 0 )
		{
			// success!
