...
BYTEARRAY UTIL_StringToBytearray( string s )
{
	if( s.size( ) < 4 )
		return  BYTEARRAY(0);
	...
	for(uint32_t i =0; i<4;i++)
	{
		...
		result.push_back( (unsigned char) UTIL_ToUInt32(Victim) );
		s=Reason;
	}
	return result;
}
...
string UTIL_HMSToString( uint32_t ms )
{
	uint32_t hours=0;
	uint32_t min=0;
	uint32_t sec=ms;
	...
	return UTIL_ToString(hours) + ":" + UTIL_ToString(min) + ":" + UTIL_ToString(sec);
}

string UTIL_MToString( uint32_t ms )
{
	uint32_t min=0;
	uint32_t sec=ms;
	for (;sec>59;)
	...
	return UTIL_ToString(min);
}

string UTIL_ToDayTimSec(uint32_t i)
{
	uint32_t hours=0;
	uint32_t min=0;
	uint32_t days=0;
	...
	return UTIL_ToString(days) + "-" + UTIL_ToString(hours) + ":" + UTIL_ToString(min) + ":" + UTIL_ToString(sec);
}
...
string UTIL_FileRead( string file, uint32_t start, uint32_t length )
{
	...
	if( IS.fail( ) )
	{
		CONSOLE_Print( "[UTIL] warning - unable to read file part [" + file + "]", true );
		return string( );
	}
...
string UTIL_FileRead( string file )
{
	...
	if( IS.fail( ) )
	{
		CONSOLE_Print( "[UTIL] warning - unable to read file [" + file + "]" , true);
		return string( );
	}
...
bool UTIL_FileWrite( string file, unsigned char *data, uint32_t length )
{
	...
	if( OS.fail( ) )
	{
		CONSOLE_Print( "[UTIL] warning - unable to write file [" + file + "]" , true);
		return false;
	}
